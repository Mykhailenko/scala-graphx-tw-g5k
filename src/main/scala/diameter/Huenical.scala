  ...
  def main(args: Array[String]) {
    ...
    graph.edges.partitionsRDD.mapPartitions(iter => {
      iter.foreach(pair => {
        val partId = pair._1
        val ep = pair._2
        val vertices: Set[VertexId] = (ep.srcIds ++ ep.dstIds).toSet
        for (v <- vertices) {
          val candidates = getCandidates(v, ep.srcIds, ep.dstIds)
    ...
    def neighbours(vertex: VertexId, srcIds: Array[VertexId], dstIds: Array[VertexId]): Array[VertexId] = {
      assert(srcIds.length == dstIds.length)
      (for (i <- 0 until srcIds.length if srcIds(i) == vertex) yield dstIds(i)).toArray
    }
    def randomVertex(): VertexId = {
      0
    }
    def getCandidates(vertex: VertexId, srcIds: Array[VertexId], dstId: Array[VertexId]): Array[VertexId] = {
      neighbours(vertex, srcIds, dstId) //:+ randomVertex()
    }
