...
class CSVChartData[T] (val graphName : String, val metricName : String){

  var m: Array[(String, Array[(Int, T)])] = Array()

  def numberOfPartitioners: Int = m.size 

  def numberOfExperiments: Int = m.head._2.size 

  def addPoints(partitionerName: String, points: Array[(Int, T)]): Unit = m =  m :+ (partitionerName, points)
  ...
  def csv: String = {
    ...
    for(i <- 0 until numberOfExperiments){
      ...
      for(j <- 0 until numberOfPartitioners){
        arr(2 + i)(1 +j) = m(j)._2(i)._2.toString
      }
    }
